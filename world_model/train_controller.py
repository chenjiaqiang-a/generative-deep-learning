...
from base.es import CMAES, SimpleGA, OpenES, PEPG
from base.model import make_model, simulate
...
def initialize_settings(sigma_init=0.1, sigma_decay=0.9999, init_opt=''):
    ...
    population = num_worker * num_worker_trial
    filebase = os.path.join(config.RUN_FOLDER, 'log/' + env_name +
                            '.' + optimizer + '.' + str(num_episode) + '.' + str(population))
    controller_filebase = os.path.join(config.RUN_FOLDER, 'controller/' +
                                       env_name + '.' + optimizer + '.' + str(num_episode) + '.' + str(population))
...
def leader():
    ...
    while True:
        ...
        for current_env_name in config.train_envs:
            send_packets_to_followers(
                packet_list, current_env_name, dream_mode)
        ...
        with open(filename_current, 'wt') as out:
            current_model_params_quantized = np.array(
                es.current_param()).round(4).tolist()
            json.dump([current_model_params_quantized, -1], out,
...
def main(args):
    global env_name, optimizer, init_opt, num_episode, eval_steps, max_length, num_worker, num_worker_trial, antithetic, seed_start, retrain_mode, dream_mode, cap_time_mode
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Train policy on OpenAI Gym environment ''using pepg, ses, openes, ga, cma')
    ...
    parser.add_argument('--init_opt', type=str, default='',
                        help='which optimizer pickle file to initialise with')
